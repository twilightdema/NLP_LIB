...
with tf.device(USED_DEVICE):
  ...
  #################################################################
  # FUNCTIONS FOR LOADING SST-2 DATASET
  def read_sst2_data_file(file_path):
    data = []
    count = 0
    with open(file_path, 'r', encoding='utf-8') as fin:
      for line in fin:
  ...
  def load_sst2_data():
    data_path_train = os.path.join('dataset_sst-2', 'train.tsv')
    data_path_dev = os.path.join('dataset_sst-2', 'dev.tsv')
    data_train = read_sst2_data_file(data_path_train)
    data_dev = read_sst2_data_file(data_path_dev)
    return data_train, data_dev

  def load_encoded_sst2_data_static_word_embedding():
    encoded_data_train_path = os.path.join('dataset', 'sst2', 'train_static_word_embedding.pk')
    encoded_data_dev_path = os.path.join('dataset', 'sst2', 'dev_static_word_embedding.pk')
    ...
    data_dev = []
    for data in data_train_:
      print('Embedding: ' + str(data['input']))
      encoded_data = perform_word_embedding(data['input'], embedding_model)
      if encoded_data is not None:
        data['input_ids'] = encoded_data
        data_train.append(data)
  ...
  def load_encoded_sst2_data_spm():
    encoded_data_train_path = os.path.join('dataset', 'sst2', 'train.pk')
    encoded_data_dev_path = os.path.join('dataset', 'sst2', 'dev.pk')
    ...
    sentence_piece_processor = spm.SentencePieceProcessor()
    print('[INFO] Max Dictionary Size = ' + str(max_dict_size))
    dict_vocab_path = os.path.join('dataset', 'sst2', 'spm.vocab')
    dict_model_path = os.path.join('dataset', 'sst2', 'spm.model')

    if not os.path.exists(dict_model_path):
      print('[INFO] No SPM model file, creating...')
      # Create raw corpus file to train SPM
      raw_corpus_file = os.path.join('dataset', 'sst2', 'corpus.txt')
      with open(raw_corpus_file, 'w', encoding='utf-8') as fout:
        for data_row in data_train:
          fout.write(data_row['input'] + '\n')
  ...
  #################################################################
  # FUNCTIONS FOR LOADING QNLI DATASET
  def read_qnli_data_file(file_path):
    data = []
    i = 0
    with open(file_path, 'r', encoding='utf-8') as fin:
      for line in fin:
        ...
        columns = line.split('\t')
        data_row = {
          'id_1': columns[0].strip(),
          'id_2': columns[0].strip(),
          'label': ('1' if columns[3].strip() == 'entailment' else '0'),
          'input_1': columns[1].strip(),
          'input_2': columns[2].strip()
        }
        print(data_row['input_1'] + ', ' + data_row['input_2'] + ' => ' + data_row['label'])
        data.append(data_row)
    return data
  ...
  def load_encoded_qnli_data_static_word_embedding():
    encoded_data_train_path = os.path.join('dataset', 'qnli', 'train_static_word_embedding.pk')
    encoded_data_dev_path = os.path.join('dataset', 'qnli', 'dev_static_word_embedding.pk')
  ...
  def load_encoded_qnli_data_spm():
    encoded_data_train_path = os.path.join('dataset', 'qnli', 'train.pk')
    encoded_data_dev_path = os.path.join('dataset', 'qnli', 'dev.pk')
    ...
    data_train, data_dev = load_qnli_data()
    max_dict_size = 150
    sentence_piece_processor = spm.SentencePieceProcessor()
    print('[INFO] Max Dictionary Size = ' + str(max_dict_size))
    dict_vocab_path = os.path.join('dataset', 'qnli', 'spm.vocab')
    dict_model_path = os.path.join('dataset', 'qnli', 'spm.model')

    if not os.path.exists(dict_model_path):
      print('[INFO] No SPM model file, creating...')
      # Create raw corpus file to train SPM
      raw_corpus_file = os.path.join('dataset', 'qnli', 'corpus.txt')
      with open(raw_corpus_file, 'w', encoding='utf-8') as fout:
        for data_row in data_train:
          fout.write(data_row['input_1'] + '\n')
          fout.write(data_row['input_2'] + '\n')
  ...
  def get_dataset_loader_func(task_name):
    if task_name == 'cola':
      ...
      return load_encoded_sst2_data_spm, load_encoded_sst2_data_static_word_embedding
    elif task_name == 'qnli':
      return load_encoded_qnli_data_spm, load_encoded_qnli_data_static_word_embedding
    else:
      return None, None
