from nlp_core.model_wrapper import EncoderModelWrapper, TrainableModelWrapper, SequenceModelWrapper
...
class TransformerEncoderOnlyS2CWrapper(EncoderModelWrapper, TrainableModelWrapper, SequenceModelWrapper):
  ...
  def get_data_effected_configs(self):
    ...
    return str(self.config['max_input_length']) + super(TransformerEncoderOnlyS2CWrapper, self).get_data_effected_configs() 
  ...
  def __init__(self, config, input_data_transform, output_data_transform):    
    super(TransformerEncoderOnlyS2CWrapper, self).__init__(config, input_data_transform, output_data_transform)
  ...
  def get_output_tensors(self):
    if self.output_tensor is None:
      ...
      print('Encoder Output flatten shape = ' + str(flatten_dim))
      flatten_encoder_output = Reshape([flatten_dim])(encoder_output_tensor)

      single_token_target_layer = Dense(self.output_data_transform.num(), use_bias=False)
      self.output_tensor = single_token_target_layer(flatten_encoder_output)
  ...
  # We actually does not use this, but put shape = single token for compatibility.
  def get_prev_output_tensors(self):
    if self.prev_output_tensor is None:
      self.prev_output_tensor = Input(shape=(1,), dtype='int32')
