...
class LayerNormalization(Layer):
  ...
  def compute_output_shape(self, input_shape):
    # print("LayerNormalization -> input_shape = "+str(input_shape))  
    return input_shape

class ScaledDotProductAttention():
  def __init__(self, d_model, layer_id, attn_dropout=0.1):
...
class MultiHeadAttention():
  ...
  def __init__(self, n_head, d_model, d_k, d_v, layer_id, dropout, mode=0, use_norm=True):
    self.mode = mode
    self.n_head = n_head
    self.d_k = d_k
    self.d_v = d_v
...
class PositionwiseFeedForward():
  def __init__(self, d_hid, d_inner_hid, layer_id, dropout=0.1):
...
class EncoderLayer():
  def __init__(self, d_model, d_inner_hid, n_head, d_k, d_v, layer_id, dropout=0.1):
    self.self_att_layer = MultiHeadAttention(n_head, d_model, d_k, d_v, layer_id + '_selfattn', dropout=dropout)
    self.pos_ffn_layer  = PositionwiseFeedForward(d_model, d_inner_hid, layer_id + '_ffn', dropout=dropout)
    self.layer_id = layer_id
...
class DecoderLayer():
  def __init__(self, d_model, d_inner_hid, n_head, d_k, d_v, layer_id, dropout=0.1):
    self.self_att_layer = MultiHeadAttention(n_head, d_model, d_k, d_v, layer_id + '_selfattn', dropout=dropout)
    self.enc_att_layer  = MultiHeadAttention(n_head, d_model, d_k, d_v, layer_id + '_endeattn', dropout=dropout)
    self.pos_ffn_layer  = PositionwiseFeedForward(d_model, d_inner_hid, layer_id + '_ffn', dropout=dropout)
    self.layer_id = layer_id
...
class Transformer:
  def __init__(self, i_tokens, o_tokens, len_limit, d_model=256, \
    ...
    pos_emb = Embedding(len_limit + 1, d_emb, trainable=False, \
               weights=[GetPosEncodingMatrix(len_limit + 1, d_emb)])
  ...
  def build_model_wo_compile(self, max_input_length, active_layers=999):
    src_seq_input = Input(shape=(max_input_length,), dtype='int32')
    tgt_seq_input = Input(shape=(max_input_length,), dtype='int32')
    ...
    # Store input and output layer of original Transformer Encoder/Decoder so we can construct new model from it
    self.src_seq_input = src_seq_input
    self.tgt_seq_input = tgt_seq_input
  ...
  def compile(self, optimizer='adam', active_layers=999):
    ...
    if (hasattr(self.model, 'metrics_tensors')):
  ...
  def make_src_seq_matrix(self, input_seq):
    ...
    for i, z in enumerate(input_seq): 
      ...
      #print("Token of "+str(z)+" = "+str(self.i_tokens.id(z)))
  ...
  def decode_sequence_fast(self, input_seq, delimiter=''):
    ...
    for i in range(self.len_limit-1):
      ...
      sampled_index = np.argmax(output[0,i,:])
      print('sampled_index = '+str(sampled_index))
      #print(self.o_tokens.id2t)
      sampled_token = self.o_tokens.token(sampled_index)
      print('sampled_token = '+str(sampled_token))
      decoded_tokens.append(sampled_token)
      if sampled_index == self.o_tokens.endid(): break
      target_seq[0,i+1] = sampled_index
    return delimiter.join(decoded_tokens[:-1])
