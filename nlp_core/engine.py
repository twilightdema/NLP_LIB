...
class NLPEngine:
  ...
  def run_train_matching_federated_simulation(self, config, node_count):
    print('[INFO] Start running weight matching federated training simulation on ' + str(node_count) + ' node(s).')

    # Load some parameters of execution_config as they may have to be instruments
    execution = config['execution']
    execution_config = execution['config']    
    base_output_dir = os.path.join(execution_config['output_dir'], 'mftrain_' + str(node_count))
    if not os.path.exists(base_output_dir):
      os.makedirs(base_output_dir)
    base_epoch = execution_config['epochs']
    ...
    for epoch in range(base_epoch):
      ...
      for node_id in range(node_count):
        ...
        # Get name information from 1st local model only (as they are the same for every local model)
        if node_id == 0:
      ...
      print('[INFO] Perform federated weight aggregation for epoch: ' + str(epoch))
      for weights_list_tuple in zip(*federated_weights_list):
          new_weights.append(np.array([np.array(weights_).mean(axis=0) for weights_ in zip(*weights_list_tuple)]))
      new_weights = np.array(new_weights)
      federated_model.set_weights(new_weights)
...
def main(argv):
  ...
  elif mode.startswith('mftrain:'):
    node_count = int(mode[len('mftrain:'):])
    print('[INFO] Perform Weight Matching Federated Training Simulation on ' + str(node_count) + ' node(s).')
    engine.run_train_matching_federated_simulation(execution_config, node_count)
